import { stage, rootLayer as root, wireLayer as wire, rootLayer } from "../app-renderer.js";
...
function handleCreateFunction(e) {
    ...
    createNode(e.name + e.args, stage, root, wire, e.children);
}
...
export function createFunctionEl(key, args, stage, root, wire, children) {
    ...
    func.addEventListener("click", () => createNode(key + args, stage, root, wire, children));
    funListEl.appendChild(row);
    funListEl1.appendChild(row.cloneNode(true));
}


function createNode(key, stage, root, wire, children) {
    let info = funList.get(key);
    let node = new FunctionNode(info, stage, root, stage.getPointerPosition());
    node.name(key)
